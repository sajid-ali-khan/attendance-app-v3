...
export const AttendanceReportView = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      rebuildReport(_studentMap, [-1]); // build initial view (total only)
    } catch (err) {
      ...
      <Card>
        ...
        <div className="flex items-center gap-4 mb-6">
          <button
            ...
            className="bg-slate-800 text-white font-semibold py-2 px-6 hover:bg-slate-700 transition-colors duration-200 h-10"
          >
            Generate Report
          </button>
          {reportData.length > 0 && (
