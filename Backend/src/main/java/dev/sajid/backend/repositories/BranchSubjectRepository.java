...
public interface BranchSubjectRepository extends JpaRepository<BranchSubject, Integer>{
    ...
    @Query("select bs.subject from BranchSubject bs where bs.branch.id = :branchId and bs.semester = :semester")
    List<Subject> findSubjectsByBranchIdAndSemester(@Param("branchId") int branchId, @Param("semester") int semester);
